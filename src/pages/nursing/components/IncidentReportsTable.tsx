import type { IncidentReport } from '@/services/api/incidentReportsService';
...
export function IncidentReportsTable({
  ...
  onDelete,
  pagination,
  onPageChange }: IncidentReportsTableProps) {
  if (loading) {
    return (
  ...
  if (reports.length === 0) {
    return (
      <div className="text-center py-12 bg-zinc-50 rounded-lg border border-zinc-200">
        <p className="text-zinc-600">No incident reports found</p>
      </div>
    );
  }
            ...
            {reports.map((report) => (
              <tr key={report.id} className="hover:bg-zinc-50">
                  ...
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      report.incidentSeverity === 'CRITICAL'
                        ...
                        : report.incidentSeverity === 'SEVERE'
                        ? 'bg-orange-100 text-orange-800'
                        : report.incidentSeverity === 'MAJOR'
                        ? 'bg-yellow-100 text-yellow-800'
                        : report.incidentSeverity === 'MODERATE'
                ...
                <td className="px-4 py-4">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      report.incidentStatus === 'CLOSED'
                        ? 'bg-green-100 text-green-800'
                        : report.incidentStatus === 'RESOLVED'
                        ? 'bg-blue-100 text-blue-800'
                        : report.incidentStatus === 'UNDER_INVESTIGATION'
                        ? 'bg-yellow-100 text-yellow-800'
                ...
                <td className="px-4 py-4 text-sm text-zinc-600">{report.location}</td>
