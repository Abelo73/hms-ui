...
export function WoundCarePage() {
  ...
  const loadWoundCare = async () => {
    ...
    try {
      ...
      const data = searchTerm
        ? await woundCareService.searchWoundCare(
            searchTerm,
            pagination.page,
            pagination.size
          )
        : await woundCareService.getWoundCareByPatientIdPaginated(
            selectedPatientId,
            pagination.page,
            pagination.size
          );

      setWoundCare(data.content);
      setPagination({
        page: data.number,
        size: data.size,
  ...
  const handleFormClose = () => {
    setIsFormOpen(false);
    setEditingWoundCare(null);
  };

  const handleEdit = (wound: WoundCare) => {
    setEditingWoundCare(wound);
    setIsFormOpen(true);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this wound care record?')) {
      return;
    }

    try {
      await woundCareService.deleteWoundCare(id);
      toast.success('Wound care record deleted successfully');
      loadWoundCare();
    } catch (error) {
      console.error('Error deleting wound care record:', error);
      toast.error('Failed to delete wound care record');
    }
  };
  ...
  return (
    <MainLayout
      title="Wound Care"
      subtitle="Track and manage wound care assessments and treatments"
    >
          ...
          {selectedPatient ? (
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search patient to manage wound care..."
      ...
      {selectedPatient ? (
        ...
        <div className="text-center py-16 bg-zinc-50 rounded-lg border border-zinc-200">
          ...
          <p className="text-zinc-600 mb-4">Search for a patient above to view and manage their wound care records</p>
        </div>
      )}
      ...
      <WoundCareFormDialog
        ...
        onSuccess={loadWoundCare}
        editingWoundCare={editingWoundCare}
