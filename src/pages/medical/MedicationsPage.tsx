...
export function MedicationsPage() {
  ...
  const loadMedications = async () => {
    ...
    try {
      ...
      const data = searchTerm
        ? await medicationsService.searchPatientMedications(
            selectedPatientId,
            searchTerm,
            pagination.page,
            pagination.size
          )
        : await medicationsService.getMedicationsByPatientIdPaginated(
            selectedPatientId,
            pagination.page,
            pagination.size
          );
  ...
  return (
    <MainLayout
      pageTitle="Medications Management"
      pageAction={
            ...
            {selectedPatient ? (
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search patient to manage medications..."
        ...
        {!selectedPatientId ? (
          <div className="text-center py-16">
            ...
            <p className="text-zinc-600 max-w-md mx-auto">
              Enter a patient name, phone number, email, or medical record number above to view and manage their medications.
            </p>
          </div>
        ) : (
          <>
            <div className="text-[12px] font-medium text-zinc-500 uppercase tracking-wider px-1">
              Medications ({pagination.totalElements} total)
            </div>
            <MedicationsTable
              medications={medications}
              onEdit={handleEdit}
              onDelete={async (medication) => {
