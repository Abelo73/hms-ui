...
import { Search, Plus, GraduationCap, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function TrainingPage() {
  ...
  useEffect(() => { loadTraining(); }, [pagination.page, searchTerm]);
  ...
  const filtered = searchTerm
    ? training.filter(t =>
        `${t.trainingName} ${t.trainingType} ${t.status}`.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : training;
    ...
    >
      <div className="space-y-4">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search by training name, type, status..."
            value={searchTerm}
        ...
        {loading ? (
          <div className="w-full h-48 border border-zinc-200 rounded-lg bg-white flex items-center justify-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-zinc-500 font-medium">Loading training...</span>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="w-full h-64 border border-dashed border-zinc-300 rounded-lg bg-zinc-50/50 flex flex-col items-center justify-center text-center p-6">
            ...
            <h3 className="text-sm font-medium text-zinc-900">No training records found</h3>
            <p className="text-xs text-zinc-500 mt-1">Try adjusting your search or add a new training.</p>
          </div>
        ) : (
          <>
            <div className="text-[12px] font-medium text-zinc-500 uppercase tracking-wider px-1">
              {pagination.totalElements} trainings
            </div>
            <div className="bg-white border border-zinc-200 rounded-lg overflow-hidden">
              <table className="w-full">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Training Name</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Type</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Start Date</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">End Date</th>
                ...
                <tbody className="divide-y divide-zinc-200">
                  {filtered.map((t) => (
                    <tr key={t.id} className="hover:bg-zinc-50">
                      <td className="px-4 py-3 text-sm text-zinc-900 font-medium">{t.trainingName}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-[11px] font-medium ${
                          ...
                          t.status === 'SCHEDULED' ? 'bg-yellow-100 text-yellow-700' :
