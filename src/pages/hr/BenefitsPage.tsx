...
import { Search, Plus, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function BenefitsPage() {
  ...
  useEffect(() => { loadBenefits(); }, [pagination.page, searchTerm]);
  ...
  const loadBenefits = async () => {
    ...
    try {
      ...
      toast.error('Failed to load benefits records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = searchTerm
    ? benefits.filter(b =>
        `${b.employeeId} ${b.benefitType} ${b.status}`.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : benefits;
    ...
    >
      <div className="space-y-4">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search by employee ID, benefit type, status..."
            value={searchTerm}
        ...
        {loading ? (
          <div className="w-full h-48 border border-zinc-200 rounded-lg bg-white flex items-center justify-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-zinc-500 font-medium">Loading benefits...</span>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="w-full h-64 border border-dashed border-zinc-300 rounded-lg bg-zinc-50/50 flex flex-col items-center justify-center text-center p-6">
            ...
            <h3 className="text-sm font-medium text-zinc-900">No benefits records found</h3>
            <p className="text-xs text-zinc-500 mt-1">Try adjusting your search or add a new benefit.</p>
          </div>
        ) : (
          <>
            <div className="text-[12px] font-medium text-zinc-500 uppercase tracking-wider px-1">
              {pagination.totalElements} benefits
            </div>
              ...
              <table className="w-full">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Benefit Type</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Plan Name</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Provider</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-200">
                  {filtered.map((benefit) => (
                    <tr key={benefit.id} className="hover:bg-zinc-50">
                      <td className="px-4 py-3 text-sm text-zinc-900">{benefit.employeeId}</td>
                      <td className="px-4 py-3 text-sm text-zinc-600">{benefit.benefitType}</td>
                      <td className="px-4 py-3 text-sm text-zinc-600">{benefit.planName || '-'}</td>
                      <td className="px-4 py-3 text-sm text-zinc-600">{benefit.provider || '-'}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-[11px] font-medium ${
                          benefit.status === 'ACTIVE' ? 'bg-green-100 text-green-700' :
                          benefit.status === 'INACTIVE' ? 'bg-red-100 text-red-700' :
