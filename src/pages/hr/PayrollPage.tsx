...
import { Search, Plus, DollarSign, ChevronLeft, ChevronRight, Play } from 'lucide-react';
...
export function PayrollPage() {
  ...
  useEffect(() => { loadPayroll(); }, [pagination.page, searchTerm]);
  ...
  const handleProcess = async (id: string) => {
    try {
      await hrService.processPayroll(id);
  ...
  const filtered = searchTerm
    ? payroll.filter(p =>
        `${p.employeeId} ${p.payPeriodStart} ${p.status}`.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : payroll;
    ...
    >
      <div className="space-y-4">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search by employee ID, period, status..."
            value={searchTerm}
        ...
        {loading ? (
          <div className="w-full h-48 border border-zinc-200 rounded-lg bg-white flex items-center justify-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-zinc-500 font-medium">Loading payroll...</span>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="w-full h-64 border border-dashed border-zinc-300 rounded-lg bg-zinc-50/50 flex flex-col items-center justify-center text-center p-6">
            ...
            <h3 className="text-sm font-medium text-zinc-900">No payroll records found</h3>
            <p className="text-xs text-zinc-500 mt-1">Try adjusting your search or add a new payroll record.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Period</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Gross Pay</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Net Pay</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Status</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-200">
                  {filtered.map((record) => (
                    <tr key={record.id} className="hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3 text-sm text-zinc-900 font-medium">${record.netPay.toFixed(2)}</td>
