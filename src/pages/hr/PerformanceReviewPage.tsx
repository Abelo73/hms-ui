...
export function PerformanceReviewPage() {
  ...
  useEffect(() => { loadReviews(); }, [pagination.page, searchTerm]);
  ...
  const filtered = searchTerm
    ? reviews.filter(r =>
        `${r.employeeId} ${r.reviewerId} ${r.rating}`.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : reviews;
    ...
    >
      <div className="space-y-4">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search by employee ID, reviewer ID, rating..."
            value={searchTerm}
        ...
        {loading ? (
          <div className="w-full h-48 border border-zinc-200 rounded-lg bg-white flex items-center justify-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-zinc-500 font-medium">Loading reviews...</span>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="w-full h-64 border border-dashed border-zinc-300 rounded-lg bg-zinc-50/50 flex flex-col items-center justify-center text-center p-6">
            ...
            <h3 className="text-sm font-medium text-zinc-900">No performance reviews found</h3>
            <p className="text-xs text-zinc-500 mt-1">Try adjusting your search or add a new review.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead className="bg-zinc-50 border-b border-zinc-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Reviewer ID</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Review Period</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Rating</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">Review Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-200">
                  {filtered.map((review) => (
                    <tr key={review.id} className="hover:bg-zinc-50">
                      <td className="px-4 py-3 text-sm text-zinc-900">{review.employeeId}</td>
                      <td className="px-4 py-3 text-sm text-zinc-600">{review.reviewerId}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center px-2 py-1 rounded-full text-[11px] font-medium bg-zinc-100 text-zinc-700">
                          <Star className="w-3 h-3 mr-1 fill-zinc-700" /> {review.rating}
                        </span>
                      </td>
